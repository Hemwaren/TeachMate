...
export default function LoginPage() {
  ...
  async function sendReset() {
    ...
    // the link will now land on our dedicated reset-password page
    setLoading(true);
    setMsg(null);
    try {
      if (!email) {
      ...
      if (error) {
        ...
        setMsg("✅ If that account exists, check your mailbox for a reset link.");
        setForgot(false);
      }
    } catch (err: any) {
      setLoading(false);
  ...
  // form submit now handles forgot mode as well
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (forgot) {
      await sendReset();
    } else {
      isSignUp ? await signUp() : await signIn(e);
    }
  };

  // parse OAuth redirect from Supabase and other URL errors (reset link)
  useEffect(() => {
    // show any error_description that came in query params (eg. otp_expired)
    const url = new URL(window.location.href);
    const errDesc = url.searchParams.get("error_description") || url.searchParams.get("error");
    if (errDesc) {
      setMsg(decodeURIComponent(errDesc));
      // remove query so message doesn't persist on reload
      url.search = "";
    ...
    // Only run OAuth handler if we actually got a token payload in the URL.
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              */}
              ...
              {/* --- Core Inputs: Email & Password (or reset) --- */}
                ...
                {forgot || !isSignUp ? (
                  <div className="text-right text-xs">
                    <button
                      ...
                      className="text-blue-200 dark:text-[#b47af5] hover:underline hover:text-blue-100"
                    >
              ...
              <div className="mt-6">
                ...
                <div className="flex gap-4 mt-4">
                  ...
                  <button type="button" disabled={forgot} className="flex-1 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 dark:bg-[#2b2b36] dark:hover:bg-[#363644] border border-white/20 dark:border-transparent text-white text-sm font-medium transition-colors flex items-center justify-center gap-2">
